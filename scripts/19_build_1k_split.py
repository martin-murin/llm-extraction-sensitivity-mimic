"""
Builds the methodology 1k split.

Reads: data/splits/methodology_1k.csv, data/splits/SPLITS_MANIFEST.json, codex_outputs/22_methodology_1k_split_build.md.
Writes: data/splits/methodology_1k.csv, data/splits/SPLITS_MANIFEST.json, codex_outputs/22_methodology_1k_split_build.md.
Backs the paper's extraction, QA, agreement, or reproducibility pipeline.
Usage: `python scripts/19_build_1k_split.py` unless the script's argparse help says otherwise.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build frozen methodology 1k split.")
    parser.add_argument("--seed", type=int, default=43)
    parser.add_argument("--n-notes", type=int, default=1000)
    parser.add_argument("--output", default="data/splits/methodology_1k.csv")
    parser.add_argument("--manifest", default="data/splits/SPLITS_MANIFEST.json")
    parser.add_argument(
        "--report",
        default="codex_outputs/22_methodology_1k_split_build.md",
    )
...
def main() -> int:
    ...
    hadm_ids = set(pd.to_numeric(stratified["hadm_id"], errors="coerce").astype("int64").tolist())
    ...
    checksums["methodology_1k"] = checksum
    checksums[output_path.name] = checksum
    manifest["checksums_sha256"] = checksums
    manifest["methodology_1k"] = {
        "generated_at_utc": datetime.now(tz=UTC).isoformat(),
        "seed": int(args.seed),
        "n_notes": int(args.n_notes),
        "source": "candidates_excluding_refinement_holdout_smoke",
    }
    ...
    summary_rows: list[dict[str, Any]] = [
        ...
        {"metric": "zero_overlap_with_existing_500", "value": overlap_ok},
        ...
        {"metric": "sha256", "value": checksum},
    ]

    lines = [
        "# Methodology 1k Split Build",
        "",
        "## Summary",
        _markdown_table(summary_rows, ["metric", "value"]),
        "",
        "## Chapter distribution check (population vs methodology_1k)",
        _markdown_table(
            chapter_rows,
    ...
    if not unique_ok:
        raise RuntimeError(
            "Methodology split unique-count check failed: "
            f"expected {args.n_notes}, got {unique_count}"
        )
    if not overlap_ok:
        raise RuntimeError(f"Methodology split overlaps existing 500-note set: {overlap_count}")
    if not chapter_ok:
        raise RuntimeError("Methodology split chapter-distribution tolerance failed (>2pp).")
