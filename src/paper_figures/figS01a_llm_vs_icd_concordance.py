...
# Builds publication figure module `figS01a_llm_vs_icd_concordance`.
#
...
# Usage: `python -m src.paper_figures.figS01a_llm_vs_icd_concordance` or `python scripts/build_paper_figures.py`.
...
class FigConfig:
    figure_name: str = "paper_fig_S01a_llm_vs_icd_concordance"
    width_inches: float = FULL_PAGE_WIDTH * 1.45
    height_inches: float = 5.0

    # Bar aesthetics
    bar_width: float = 0.36
    color_p_llm_given_lf: str = COLOR_SIGNAL_LLM
    color_p_lf_given_llm: str = COLOR_SIGNAL_ICD_LF
...
@lru_cache(maxsize=1)
def _compute_icd_concordance_frame() -> pd.DataFrame:
    try:
        return _compute_icd_concordance_frame_db()
    except Exception as exc:  # pragma: no cover - explicit hard fail on DB issues
        raise RuntimeError(
            "Figure S01a requires live DB connectivity for ICD-LF concordance; "
            "markdown fallback has been removed."
        ) from exc


def _compute_icd_concordance_frame_db() -> pd.DataFrame:
    parsed_by_variant = {
    ...
    frame["target_label"] = (
        frame["target"]
        .str.replace("admission_reason_tags::", "", regex=False)
...
def render(df: pd.DataFrame, cfg: FigConfig = CFG) -> plt.Figure:
    ...
    ax.set_xticks(x)
    wrapped_labels = [
        textwrap.fill(str(lbl), width=cfg.label_wrap_width)
        for lbl in df["target_label"].tolist()
    ]
    ax.set_xticklabels(
        wrapped_labels,
        rotation=65,
...
def build() -> plt.Figure:
    df = _compute_icd_concordance_frame()
    fig = render(df)
